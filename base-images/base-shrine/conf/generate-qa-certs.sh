...
openssl req -x509 -new -newkey rsa:2048 -keyout $SHRINE_HTTPS_ROOT/$SHRINE_HTTPS_ROOT.key -sha256 -nodes -days 1024 -out $SHRINE_HTTPS_ROOT/$SHRINE_HTTPS_ROOT.crt -subj "/C=US/ST=Massachusetts/L=Worcester/O=UMass Medical School/OU=DST Shrine/CN=$SHRINE_HTTPS_ROOT"

# Generate the hub CA
openssl req -x509 -new -newkey rsa:2048 -keyout $SHRINE_HUB/$SHRINE_HUB.key -sha256 -nodes -days 1024 -out $SHRINE_HUB/$SHRINE_HUB.crt -subj "/C=US/ST=Massachusetts/L=Worcester/O=UMass Medical School/OU=DST Shrine/CN=$SHRINE_HUB"

for HOST in ${QA_HOSTS[*]}; do
  mkdir -p $HOST
  # Generate the query signing certificate CSR
  openssl req -new -newkey rsa:2048 -keyout $HOST/$HOST.catalyst.key -sha256 -nodes -days 1024 -out $HOST/$HOST.catalyst.csr -config openssl.cnf -subj "/C=US/ST=Massachusetts/L=Worcester/O=UMass Medical School/OU=DST Shrine/CN=$HOST.catalyst"
  # Generate the https serving certificate CSR
  openssl req -new -newkey rsa:2048 -keyout $HOST/$HOST-https.catalyst.key -sha256 -nodes -days 1024 -out $HOST/$HOST-https.catalyst.csr -config openssl.cnf -subj "/C=US/ST=Massachusetts/L=Worcester/O=UMass Medical School/OU=DST Shrine/CN=$HOST-https.catalyst"
