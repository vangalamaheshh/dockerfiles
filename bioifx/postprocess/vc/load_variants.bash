...
for dataset_id in $(gcloud alpha genomics datasets list | grep ${project_id} | grep -oP "^\S+"); do
...
bq_op_id=$(gcloud alpha genomics variantsets export ${variantset_id} ${table_name} --bigquery-dataset ${BQ_GENOMICS_DATASET} 2>&1 | grep -oP "name:\s+(.+)" | sed -e "s/name:\s*//g")
