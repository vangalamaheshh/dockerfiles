...
print_matrix( $matrix, $options, $files, $meta_info, $$options{'out_file'});
exit $?;


sub parse_options {
	my $options = {};
	GetOptions( $options, 'file|f=s@', 'column|l:i', 'htseq|t', 'cufflinks|c', 
    'out_file|o=s', 'remove_ERCC_ids|e', 'header|d', 'metasheet|m:s', 'help|h' );
	unless( $$options{ 'file' } or $$options{'out_file'}) {
		my $usage = "$0 <file|-f> <--out_file|-o> [--file|-f] [--column|-l <1 or 2 or 3; default=3>] [--htseq|-t] 
...
sub get_matrix {
    ...
    $file_base =~ s/^\d+\-//;
		if( $$options{ 'cufflinks' } ) {
			$file_base =~ s/\.genes\.fpkm_tracking//;
		} elsif( $$options{ 'htseq' } ) {
...
sub print_matrix {
  ...
  print OFH join(",", ("Gene_ID",@header)), "\n";
	foreach my $gene_id( keys %$matrix ) {
		...
		print OFH join( ",", ( $gene_id, @counts ) ), "\n";
	}
  close OFH or die "Error in closing the file handle, $out_file, $!\n";
}
